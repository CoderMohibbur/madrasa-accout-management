@php
    ...
    $boardingListUrl = \Illuminate\Support\Facades\Route::has('boarding.students.index')
        ? route('boarding.students.index')
        : null;

    $boardingUpdateUrl = \Illuminate\Support\Facades\Route::has('students.boarding.update')
        ? route('students.boarding.update', $student->id)
        : null;
    ...
    // academic label safe
    $yearLabel =
        data_get($student, 'academicYear.year') ??
        (data_get($student, 'academicYear.name') ??
            (data_get($student, 'academicYear.title') ??
                ($student->academic_year_id ? 'Year #' . $student->academic_year_id : '-')));

    $classLabel =
        data_get($student, 'class.name') ??
        (data_get($student, 'class.class_name') ?? ($student->class_id ? 'Class #' . $student->class_id : '-'));

    $sectionLabel =
        data_get($student, 'section.name') ??
        (data_get($student, 'section.section_name') ??
            ($student->section_id ? 'Section #' . $student->section_id : '-'));

    $feesTypeLabel =
        data_get($student, 'feesType.name') ??
        (data_get($student, 'feesType.title') ??
            ($student->fees_type_id ? 'FeesType #' . $student->fees_type_id : '-'));

    $boardingStartVal = old(
        'boarding_start_date',
        $student->boarding_start_date?->format('Y-m-d') ?? ($student->boarding_start_date ?? ''),
    );

    $boardingEndVal = old(
        'boarding_end_date',
        $student->boarding_end_date?->format('Y-m-d') ?? ($student->boarding_end_date ?? ''),
    );

    $boardingNoteVal = old('boarding_note', $student->boarding_note ?? '');
@endphp
...
<x-app-layout>
    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-4">
            ...
            {{-- Hero --}}
            <div
                class="rounded-3xl border border-slate-200 dark:border-white/10 bg-gradient-to-r from-slate-900 to-slate-700 p-5 text-white shadow-sm">
                <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div class="flex items-center gap-3 min-w-0">
                        <div
                            class="h-12 w-12 rounded-2xl bg-white/10 border border-white/10 flex items-center justify-center shrink-0 overflow-hidden">
                            @if ($photoUrl)
                    ...
                    <div class="flex items-center gap-2 flex-wrap">
                        ...
                        @if ($student->isActived)
                            <span
                                class="inline-flex items-center gap-1 rounded-full bg-emerald-500/15 border border-emerald-400/20 px-3 py-1 text-xs font-semibold">
                                <span class="h-1.5 w-1.5 rounded-full bg-emerald-400"></span> Active
                            </span>
                        @else
                            <span
                                class="inline-flex items-center gap-1 rounded-full bg-rose-500/15 border border-rose-400/20 px-3 py-1 text-xs font-semibold">
                        ...
                        @if ($student->is_boarding)
                            <span
                                class="inline-flex items-center gap-1 rounded-full bg-white/10 border border-white/10 px-3 py-1 text-xs font-semibold">
                                🛏️ Boarding
                            </span>
                        @else
                            <span
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div
                    class="rounded-3xl border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-900 p-4 shadow-sm">
                    <div class="text-[11px] text-slate-500 dark:text-slate-400">Total Paid (All time)</div>
                    <div class="mt-1 text-lg font-bold text-slate-900 dark:text-slate-100">{{ $money($feeTotalPaid) }}
                    </div>
                </div>
                ...
                <div
                    ...
                    <div class="mt-1 text-lg font-bold text-emerald-700 dark:text-emerald-300">
                        {{ $money($feePaidInRange) }}</div>
                    <div class="mt-1 text-[11px] text-slate-500 dark:text-slate-400">{{ $fromVal }} →
                        {{ $toVal }}</div>
                </div>
                ...
                <div
                    ...
                    <div class="mt-1 text-lg font-bold text-rose-700 dark:text-rose-300">{{ $money($feeTotalDue) }}
                    </div>
                </div>
                ...
                <div
                    ...
                    <div class="mt-1 text-lg font-bold text-rose-700 dark:text-rose-300">{{ $money($feeDueInRange) }}
                    </div>
                    <div class="mt-1 text-[11px] text-slate-500 dark:text-slate-400">{{ $fromVal }} →
                        {{ $toVal }}</div>
                </div>
            ...
            <div class="grid grid-cols-12 gap-3">
                ...
                <div class="col-span-12 lg:col-span-5 space-y-3">
                    ...
                    {{-- Basic Info --}}
                    <div
                        class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-3xl shadow-sm overflow-hidden">
                        <div class="px-4 py-3 border-b border-slate-200 dark:border-white/10">
                            <div class="text-sm font-semibold text-slate-900 dark:text-slate-100">Basic Information
                            </div>
                        ...
                        <div class="p-4 grid grid-cols-2 gap-3 text-sm">
                            ...
                            <div class="rounded-2xl border border-slate-200 dark:border-white/10 p-3">
                                ...
                                <div class="font-semibold text-slate-900 dark:text-slate-100">
                                    {{ $student->father_name ?? '-' }}</div>
                            </div>

                            <div class="rounded-2xl border border-slate-200 dark:border-white/10 p-3">
                                <div class="text-[11px] text-slate-500 dark:text-slate-400">DOB</div>
                                <div class="font-semibold text-slate-900 dark:text-slate-100">
                                    {{ $student->dob?->format('Y-m-d') ?? ($student->dob ?? '-') }}</div>
                            </div>
                    ...
                    <div
                        ...
                        <div class="px-4 py-3 border-b border-slate-200 dark:border-white/10">
                            ...
                            <div class="text-xs text-slate-500 dark:text-slate-400">Year • Class • Section • Fees type
                            </div>
                        </div>
                        ...
                        <div class="p-4 grid grid-cols-2 gap-3 text-sm">
                            <div class="rounded-2xl border border-slate-200 dark:border-white/10 p-3">
                                ...
                                <div class="font-semibold text-slate-900 dark:text-slate-100">{{ $yearLabel }}</div>
                            </div>
                            ...
                            <div class="rounded-2xl border border-slate-200 dark:border-white/10 p-3">
                                ...
                                <div class="font-semibold text-slate-900 dark:text-slate-100">{{ $classLabel }}</div>
                            </div>

                            <div class="rounded-2xl border border-slate-200 dark:border-white/10 p-3">
                                <div class="text-[11px] text-slate-500 dark:text-slate-400">Section</div>
                                <div class="font-semibold text-slate-900 dark:text-slate-100">{{ $sectionLabel }}</div>
                            </div>

                            <div class="rounded-2xl border border-slate-200 dark:border-white/10 p-3">
                                <div class="text-[11px] text-slate-500 dark:text-slate-400">Fees Type</div>
                                <div class="font-semibold text-slate-900 dark:text-slate-100">{{ $feesTypeLabel }}
                                </div>
                            </div>
                            ...
                            <div class="rounded-2xl border border-slate-200 dark:border-white/10 p-3">
                                ...
                                <div class="font-semibold text-slate-900 dark:text-slate-100">
                                    {{ $money($student->scholarship_amount) }}</div>
                            </div>
                        </div>
                    </div>

                    {{-- Boarding (VIEW + TOGGLE FORM) --}}
                    <div
                        class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-3xl shadow-sm overflow-hidden">
                        <div class="px-4 py-3 border-b border-slate-200 dark:border-white/10">
                            <div class="text-sm font-semibold text-slate-900 dark:text-slate-100">Boarding</div>
                            <div class="text-xs text-slate-500 dark:text-slate-400">Hostel status & dates (toggle from
                                here)</div>
                        </div>

                        <div class="p-4 space-y-3 text-sm">
                            <div class="flex items-center justify-between">
                                <div class="text-slate-600 dark:text-slate-300">Current Status</div>
                                @if ($student->is_boarding)
                                    <span
                            ...
                            @if ($student->is_boarding)
                                ...
                                <div class="rounded-2xl border border-slate-200 dark:border-white/10 p-3">
                                    ...
                                    <div class="font-semibold text-slate-900 dark:text-slate-100">
                                        {{ $student->boarding_note ?? '-' }}</div>
                                </div>
                            @endif

                            {{-- Toggle form --}}
                            @if ($boardingUpdateUrl)
                                <div class="rounded-2xl border border-slate-200 dark:border-white/10 p-3">
                                    <div class="text-xs text-slate-500 dark:text-slate-400 mb-2">
                                        Update boarding status (ON করলে start date না দিলে auto আজকের তারিখ হবে)
                                    </div>

                                    <form method="POST" action="{{ $boardingUpdateUrl }}" class="space-y-2"
                                        id="boardingForm">
                                        @csrf
                                        @method('PATCH')

                                        <label
                                            class="flex items-center gap-2 text-sm font-medium text-slate-900 dark:text-slate-100">
                                            <input type="hidden" name="is_boarding" value="0">
                                            <input type="checkbox" name="is_boarding" value="1"
                                                id="boardingToggle" @checked((bool) $student->is_boarding)
                                                class="rounded border-slate-200 dark:border-white/10 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500">
                                            Boarding Enable
                                        </label>
                                        ...
                                        <div class="grid grid-cols-2 gap-2">
                                            <div>
                                                <label
                                                    class="block text-[11px] text-slate-500 dark:text-slate-400">Start
                                                    Date</label>
                                                <input type="date" name="boarding_start_date"
                                                    value="{{ $boardingStartVal }}" id="boardingStart"
                                                    class="w-full rounded border-slate-200 dark:border-white/10 bg-white dark:bg-slate-950 px-3 py-2 text-sm text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500">
                                            </div>
                                            <div>
                                                <label class="block text-[11px] text-slate-500 dark:text-slate-400">End
                                                    Date</label>
                                                <input type="date" name="boarding_end_date"
                                                    value="{{ $boardingEndVal }}" id="boardingEnd"
                                                    class="w-full rounded border-slate-200 dark:border-white/10 bg-white dark:bg-slate-950 px-3 py-2 text-sm text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500">
                                            </div>
                                        </div>
                                        ...
                                        <div>
                                            <label
                                                class="block text-[11px] text-slate-500 dark:text-slate-400">Note</label>
                                            <input type="text" name="boarding_note"
                                                value="{{ $boardingNoteVal }}" id="boardingNote"
                                                placeholder="e.g. Hostel assigned by admin"
                                                class="w-full rounded border-slate-200 dark:border-white/10 bg-white dark:bg-slate-950 px-3 py-2 text-sm text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500">
                                        </div>
                                ...
                                <script>
                                    (function() {
                                        const toggle = document.getElementById('boardingToggle');
                                        const start = document.getElementById('boardingStart');
                                        const end = document.getElementById('boardingEnd');
                                        const note = document.getElementById('boardingNote');

                                        if (!toggle) return;

                                        const apply = () => {
                                            const on = !!toggle.checked;
                                            // enable/disable inputs for UX (backend also enforces logic)
                            ...
                            @else
                                <div class="text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-xl p-3">
                                    Boarding update route not found. Please add route <b>students.boarding.update</b> to
                                    enable toggle.
                                </div>
                            @endif
                        </div>
                ...
                <div class="col-span-12 lg:col-span-7 space-y-3">
                    ...
                    <div
                        ...
                        <div
                            ...
                            <div>
                                <div class="text-sm font-semibold text-slate-900 dark:text-slate-100">Fee History
                                    (Month-wise)</div>
                                <div class="text-xs text-slate-500 dark:text-slate-400">Collected fee summary by month
                                </div>
                            </div>
                        ...
                        <div class="overflow-x-auto">
                            <table class="min-w-full text-sm">
                                <thead
                                    class="bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-200 text-xs uppercase">
                                    <tr>
                                ...
                                <tbody class="divide-y divide-slate-100 dark:divide-white/10">
                                    @forelse($feeHistory as $row)
                                        @php
                                            $monthName =
                                                data_get($row, 'month.name') ??
                                                (data_get($row, 'month.month_name') ??
                                                    (data_get($row, 'month_title') ??
                                                        'Month #' .
                                                            (data_get($row, 'months_id') ??
                                                                (data_get($row, 'month_id') ?? ''))));
                                            $lastDate =
                                                data_get($row, 'last_date') ??
                                                (data_get($row, 'transactions_date') ?? '-');
                                            $paid =
                                                (float) (data_get($row, 'total_paid') ?? (data_get($row, 'paid') ?? 0));
                                        @endphp
                                        <tr class="hover:bg-slate-50/70 dark:hover:bg-slate-800/60">
                                            ...
                                            <td class="px-4 py-3 text-slate-600 dark:text-slate-300 whitespace-nowrap">
                                                {{ $lastDate }}</td>
                                            <td
                                                class="px-4 py-3 text-right font-semibold text-emerald-700 dark:text-emerald-300">
                                                {{ $money($paid) }}</td>
                                        </tr>
                                    @empty
                                        <tr>
                    ...
                    <div
                        ...
                        <div class="px-4 py-3 border-b border-slate-200 dark:border-white/10">
                            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                                <div>
                                    <div class="text-sm font-semibold text-slate-900 dark:text-slate-100">Transaction
                                        History</div>
                                    <div class="text-xs text-slate-500 dark:text-slate-400">All transactions of this
                                        student</div>
                                </div>
                                ...
                                <div class="flex flex-col sm:flex-row sm:items-end gap-2">
                                    <form method="GET" action="{{ route('students.show', $student) }}"
                                        ...
                                        <div>
                                            <label
                                                class="block text-[11px] text-slate-500 dark:text-slate-400">From</label>
                                        ...
                                        <div>
                                            <label
                                                class="block text-[11px] text-slate-500 dark:text-slate-400">To</label>
                        ...
                        <div class="overflow-x-auto">
                            <table class="min-w-full text-sm">
                                ...
                                <tbody class="divide-y divide-slate-100 dark:divide-white/10">
                                    @php $rows = $txs ? $txs : collect(); @endphp
                                    ...
                                    @forelse($rows as $tx)
                                        ...
                                        <tr class="hover:bg-slate-50/70 dark:hover:bg-slate-800/60">
                                            ...
                                            <td
                                                class="px-4 py-3 text-right font-semibold {{ $debit > 0 ? 'text-rose-700 dark:text-rose-300' : 'text-slate-400' }}">
                                                {{ $debit > 0 ? $money($debit) : '-' }}
                                            </td>

                                            <td
                                                class="px-4 py-3 text-right font-semibold {{ $credit > 0 ? 'text-emerald-700 dark:text-emerald-300' : 'text-slate-400' }}">
                        ...
                        @if ($txs && method_exists($txs, 'links'))
                            <div
                                class="border-t border-slate-200 dark:border-white/10 bg-slate-50/60 dark:bg-slate-800/40 px-4 py-3">
                                {{ $txs->links() }}
                            </div>
                        @endif
                    </div>
