...
@php
    ...
    $getParty = function ($tx) {
        ...
        if (!empty($tx->student_id)) return ['label' => 'Student', 'name' => 'ID: ' . $tx->student_id];
        if (!empty($tx->doner_id)) return ['label' => 'Donor', 'name' => 'ID: ' . $tx->doner_id];
        if (!empty($tx->lender_id)) return ['label' => 'Lender', 'name' => 'ID: ' . $tx->lender_id];
    ...
    $classDefaultFeesEndpoint = $classDefaultFeesEndpoint ?? (Route::has('ajax.class_default_fees') ? route('ajax.class_default_fees') : url('/ajax/class-default-fees'));
@endphp
...
<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between gap-4">
            <div>
                <h2 class="font-semibold text-xl text-slate-800 dark:text-slate-100 leading-tight">{{ $pageTitle }}</h2>
    ...
    <div class="py-6" x-data="transactionCenterPage()" x-init="init()">
        ...
        <div x-show="toast.show" x-cloak
            class="fixed z-50 right-4 bottom-4 max-w-md w-[calc(100vw-2rem)] sm:w-auto">
            <div
                class="rounded-2xl shadow-xl border px-4 py-3 text-sm"
                ...
                <div class="flex items-start gap-3">
                    ...
                    <button type="button" class="text-xs opacity-70 hover:opacity-100" @click="toast.show=false">✕</button>
                </div>
            </div>
        </div>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid grid-cols-12 gap-4">
                ...
                <section class="col-span-12 lg:col-span-8">
                    ...
                    {{-- Table --}}
                    <div class="mt-4 bg-white dark:bg-slate-950 border border-slate-200 dark:border-white/10 rounded-2xl shadow-sm overflow-hidden">
                        ...
                        <div class="overflow-x-auto">
                            <table class="min-w-full text-sm">
                                <thead class="bg-slate-50 dark:bg-white/5 text-slate-600 dark:text-slate-300 text-xs uppercase">
                                    <tr>
                                ...
                                <tbody class="divide-y divide-slate-100 dark:divide-white/10">
                                    @forelse(($transactions ?? []) as $tx)
                                            ...
                                            $typeName =
                                                ...
                                                (data_get($tx, 'transactionsType.name') ??
                                                    (data_get($tx, 'transactions_type.name') ??
                                                        ($tx->transactions_type_id ?? null ? 'Type #' . $tx->transactions_type_id : '-')));

                                            $accountName =
                                                data_get($tx, 'account.name') ?? ($tx->account_id ?? null ? 'Account #' . $tx->account_id : '-');
                                        ...
                                        <tr class="hover:bg-slate-50/70 dark:hover:bg-white/5">
                                            ...
                                            <td class="px-4 py-3">
                                                <span class="inline-flex items-center rounded-lg border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-900 px-2 py-1 text-xs text-slate-700 dark:text-slate-200">
                                                    {{ $typeName }}
                                                </span>
                                            </td>

                                            <td class="px-4 py-3">
                                                <div class="font-medium text-slate-800 dark:text-slate-100">{{ $party['name'] }}</div>
                                                <div class="text-xs text-slate-500">{{ $party['label'] }}</div>
                                            </td>

                                            <td class="px-4 py-3">
                                                <div class="text-slate-800 dark:text-slate-100">{{ $accountName }}</div>
                                                <div class="text-xs text-slate-500">
                ...
                <aside class="col-span-12 lg:col-span-4">
                    ...
                    <div x-data="transactionCenterQuickAdd(@js($activeTab), @js($feeMode))"
                        ...
                        @if ($errors->any())
                            <div class="mx-4 mt-3 rounded-xl border border-rose-200 bg-rose-50 px-3 py-2 text-rose-800 text-xs">
                        ...
                        <div class="px-4 pb-4 space-y-4">
                            ...
                            <div x-show="tab==='student_fee'" x-cloak class="space-y-3">
                                ...
                                <div class="grid grid-cols-2 gap-2">
                                    <button type="button" @click="feeMode='single'" class="rounded-xl border px-3 py-2 text-xs"
                                    ...
                                    <button type="button" @click="feeMode='bulk'" class="rounded-xl border px-3 py-2 text-xs"
                                        :class="feeMode === 'bulk'
                                            ? 'bg-slate-900 text-white border-slate-900'
                                            : 'bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-200 border-slate-200 dark:border-white/10 hover:bg-slate-50 dark:hover:bg-white/5'">
                                        Bulk Students
                                ...
                                {{-- SINGLE --}}
                                <div x-show="feeMode==='single'" x-cloak class="rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-950 p-3">
                                    <div class="mb-2">
                                        <div class="text-sm font-semibold text-slate-800 dark:text-slate-100">Single Fee</div>
                                        <div class="text-xs text-slate-500">One student fee entry</div>
                                    </div>

                                    <form id="tc_single_fee_form" method="POST" action="{{ url('/transactions/quick-store') }}" class="space-y-3">
                                        @csrf
                                        <input type="hidden" name="type_key" value="student_fee">
                                        <input type="hidden" name="_tc_fee_mode" value="single">

                                        <div>
                                            <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Student</label>
                                            <div class="flex items-center gap-2">
                                                <select id="select_student_fee_student" data-entity-select="students" name="student_id" class="{{ $selectClass }}">
                                                ...
                                                <button type="button" @click="openModal('students', 'select_student_fee_student')"
                                        ...
                                        <div class="grid grid-cols-2 gap-2">
                                            <div>
                                                ...
                                                <select id="select_student_fee_year" name="academic_year_id" class="{{ $selectClass }}">
                                                    <option value="">Select year</option>
                                                    @foreach ($years ?? [] as $y)
                                                        <option value="{{ $y->id }}" @selected($oldSelected('academic_year_id', $y->id, 'student_fee'))>
                                                            {{ $y->name ?? 'Year #' . $y->id }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                            ...
                                            <div>
                                                <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Month <span class="text-rose-600">*</span></label>
                                                <select id="select_student_fee_month" name="months_id" class="{{ $selectClass }}">
                                                    <option value="">Select month</option>
                                                    @foreach ($months ?? [] as $m)
                                                        <option value="{{ $m->id }}" @selected($oldSelected('months_id', $m->id, 'student_fee'))>
                                                            {{ $m->name ?? 'Month #' . $m->id }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                        ...
                                        <div class="grid grid-cols-2 gap-2">
                                            <div>
                                                <div class="flex items-center justify-between mb-1">
                                                    <label class="block text-xs font-medium text-slate-600 dark:text-slate-300">Class</label>
                                                ...
                                                <select id="select_student_fee_class" name="class_id" class="{{ $selectClass }}">
                                                    <option value="">Select class</option>
                                                    @foreach ($classes ?? [] as $c)
                                                        <option value="{{ $c->id }}" @selected($oldSelected('class_id', $c->id, 'student_fee'))>
                                                            {{ $c->name ?? 'Class #' . $c->id }}
                                                        </option>
                                                    @endforeach
                                                </select>

                                                <p id="tc_defaults_status" class="text-[11px] text-slate-500 mt-1"></p>
                                            </div>

                                            <div>
                                                <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Section</label>
                                                <select id="select_student_fee_section" name="section_id" class="{{ $selectClass }}">
                                                    <option value="">Select section</option>
                                                    @foreach ($sections ?? [] as $sec)
                                                        <option value="{{ $sec->id }}" @selected($oldSelected('section_id', $sec->id, 'student_fee'))>
                                        ...
                                        <div class="grid grid-cols-2 gap-2">
                                            <div>
                                                <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Fees Type</label>
                                                <select id="select_student_fee_fess_type" name="fess_type_id" class="{{ $selectClass }}">
                                                    <option value="">Select fees type</option>
                                                    @foreach ($feesTypes ?? [] as $ft)
                                                        <option value="{{ $ft->id }}" @selected($oldSelected('fess_type_id', $ft->id, 'student_fee'))>
                                                            {{ $ft->name ?? 'Type #' . $ft->id }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                            ...
                                            <div>
                                                <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Roll (Auto)</label>
                                                <input id="student_fee_roll" type="text" readonly class="{{ $readonlyClass }}" placeholder="Auto from student">
                                            </div>
                                        </div>

                                        <div>
                                            <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Student Book Number</label>
                                            <input type="text" name="student_book_number" value="{{ $oldFor('student_book_number', 'student_fee') }}" class="{{ $inputClass }}">
                                        </div>

                                        {{-- Fees breakdown --}}
                                        <div class="grid grid-cols-2 gap-2">
                                            <div>
                                                <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Admission Fee</label>
                                                <input type="number" step="0.01" min="0" name="admission_fee" value="{{ $oldFor('admission_fee', 'student_fee') }}" class="tc-fee-input {{ $inputClass }}">
                                            </div>

                                            <div>
                                                <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Monthly Fee</label>
                                                <input type="number" step="0.01" min="0" name="monthly_fees" value="{{ $oldFor('monthly_fees', 'student_fee') }}" class="tc-fee-input {{ $inputClass }}">
                                            </div>

                                            <div>
                                                <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Boarding Fee</label>
                                                <input type="number" step="0.01" min="0" name="boarding_fees" value="{{ $oldFor('boarding_fees', 'student_fee') }}" class="tc-fee-input {{ $inputClass }}">
                                            </div>

                                            <div>
                                                <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Management Fee</label>
                                                <input type="number" step="0.01" min="0" name="management_fees" value="{{ $oldFor('management_fees', 'student_fee') }}" class="tc-fee-input {{ $inputClass }}">
                                            </div>

                                            <div>
                                                <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Exam Fee</label>
                                                <input type="number" step="0.01" min="0" name="exam_fees" value="{{ $oldFor('exam_fees', 'student_fee') }}" class="tc-fee-input {{ $inputClass }}">
                                            </div>

                                            <div>
                                                <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Others</label>
                                                <input type="number" step="0.01" min="0" name="others_fees" value="{{ $oldFor('others_fees', 'student_fee') }}" class="tc-fee-input {{ $inputClass }}">
                                            </div>
                                        </div>

                                        <div>
                                            <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Total Fee (Auto)</label>
                                            <input id="tc_single_total" type="number" step="0.01" min="0" name="total_fees"
                                                value="{{ $oldFor('total_fees', 'student_fee') }}" class="{{ $readonlyClass }}" readonly>
                                        ...
                                        <div>
                                            <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Account <span class="text-rose-600">*</span></label>
                                            <div class="flex items-center gap-2">
                                                <select id="select_student_fee_account" data-entity-select="accounts" name="account_id" class="{{ $selectClass }}">
                                                ...
                                                <button type="button" @click="openModal('accounts', 'select_student_fee_account')"
                                                    class="shrink-0 rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-900 px-3 py-2 text-xs hover:bg-slate-50 dark:hover:bg-white/5">
                                                    + Add
                                                </button>
                                            </div>
                                        ...
                                        <div class="grid grid-cols-2 gap-2">
                                            <div>
                                                <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Date <span class="text-rose-600">*</span></label>
                                                <input type="date" name="transactions_date"
                                                    value="{{ $oldFor('transactions_date', 'student_fee') ?: $today }}"
                                                    class="{{ $inputClass }}">
                                            </div>
                                            <div>
                                                <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Receipt No</label>
                                                <input type="text" name="recipt_no" value="{{ $oldFor('recipt_no', 'student_fee') }}" class="{{ $inputClass }}">
                                            </div>
                                        </div>

                                        <div>
                                            <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Note</label>
                                            <input type="text" name="note" value="{{ $oldFor('note', 'student_fee') }}" class="{{ $inputClass }}">
                                        </div>
                                ...
                                {{-- BULK --}}
                                <div x-show="feeMode==='bulk'" x-cloak class="rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-950 p-3">
                                    <div class="mb-2">
                                        <div class="text-sm font-semibold text-slate-800 dark:text-slate-100">Bulk Fees</div>
                            ...
                            <div x-show="tab==='donation'" x-cloak>
                                <form method="POST" action="{{ url('/transactions/quick-store') }}" class="space-y-3">
                                    ...
                                    <div>
                                        <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Donor</label>
                                        <div class="flex items-center gap-2">
                                            <select id="select_donation_donor" data-entity-select="donors" name="doner_id" class="{{ $selectClass }}">
                                                <option value="">Select donor</option>
                                                @foreach ($donors ?? [] as $d)
                                                    @php $label = $d->name ?? ($d->donor_name ?? ($d->doner_name ?? 'Donor #' . $d->id)); @endphp
                                                    <option value="{{ $d->id }}" @selected($oldSelected('doner_id', $d->id, 'donation'))>{{ $label }}</option>
                                                @endforeach
                                            </select>
                                            <button type="button" @click="openModal('donors', 'select_donation_donor')"
                                                class="shrink-0 rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-900 px-3 py-2 text-xs hover:bg-slate-50 dark:hover:bg-white/5">
                                                + Add
                                            </button>
                                        </div>
                                    ...
                                    <div>
                                        ...
                                        <input type="number" step="0.01" name="amount" value="{{ $oldFor('amount', 'donation') }}" class="{{ $inputClass }}">
                                        <p class="text-[11px] text-slate-500 mt-1">Ledger: Donation = <b>IN (credit)</b>.</p>
                                    </div>

                                    <div>
                                        <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Account <span class="text-rose-600">*</span></label>
                                        <div class="flex items-center gap-2">
                                            <select id="select_donation_account" data-entity-select="accounts" name="account_id" class="{{ $selectClass }}">
                                                <option value="">Select account</option>
                                                @foreach ($accounts ?? [] as $acc)
                                            ...
                                            <button type="button" @click="openModal('accounts', 'select_donation_account')"
                                                class="shrink-0 rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-900 px-3 py-2 text-xs hover:bg-slate-50 dark:hover:bg-white/5">
                                                + Add
                                            </button>
                                        </div>
                                    ...
                                    <div class="grid grid-cols-2 gap-2">
                                        <div>
                                            <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Date <span class="text-rose-600">*</span></label>
                                            <input type="date" name="transactions_date" value="{{ $oldFor('transactions_date', 'donation') ?: $today }}" class="{{ $inputClass }}">
                                        </div>
                                        <div>
                                            <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Receipt No</label>
                                            <input type="text" name="recipt_no" value="{{ $oldFor('recipt_no', 'donation') }}" class="{{ $inputClass }}">
                                        </div>
                                    </div>

                                    <div>
                                        <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Note</label>
                                        <input type="text" name="note" value="{{ $oldFor('note', 'donation') }}" class="{{ $inputClass }}">
                                    </div>
                            ...
                            <div x-show="tab==='loan_taken'" x-cloak>
                                <form method="POST" action="{{ url('/transactions/quick-store') }}" class="space-y-3">
                                    ...
                                    <div>
                                        <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Lender</label>
                                        <div class="flex items-center gap-2">
                                            <select id="select_loan_taken_lender" data-entity-select="lenders" name="lender_id" class="{{ $selectClass }}">
                                                <option value="">Select lender</option>
                                                @foreach ($lenders ?? [] as $l)
                                                    @php $label = $l->name ?? ($l->lender_name ?? 'Lender #' . $l->id); @endphp
                                                    <option value="{{ $l->id }}" @selected($oldSelected('lender_id', $l->id, 'loan_taken'))>{{ $label }}</option>
                                                @endforeach
                                            </select>
                                            <button type="button" @click="openModal('lenders', 'select_loan_taken_lender')"
                                                class="shrink-0 rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-900 px-3 py-2 text-xs hover:bg-slate-50 dark:hover:bg-white/5">
                                                + Add
                                            </button>
                                        </div>
                                    ...
                                    <div>
                                        <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Amount</label>
                                        <input type="number" step="0.01" name="amount" value="{{ $oldFor('amount', 'loan_taken') }}" class="{{ $inputClass }}">
                                        <p class="text-[11px] text-slate-500 mt-1">Ledger: Loan Taken = <b>IN (credit)</b>.</p>
                                    </div>

                                    <div>
                                        <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Account <span class="text-rose-600">*</span></label>
                                        <div class="flex items-center gap-2">
                                            <select id="select_loan_taken_account" data-entity-select="accounts" name="account_id" class="{{ $selectClass }}">
                                                <option value="">Select account</option>
                                                @foreach ($accounts ?? [] as $acc)
                                                    <option value="{{ $acc->id }}" @selected($oldSelected('account_id', $acc->id, 'loan_taken'))>{{ $acc->name ?? 'Account #' . $acc->id }}</option>
                                                @endforeach
                                            </select>
                                            <button type="button" @click="openModal('accounts', 'select_loan_taken_account')"
                                                class="shrink-0 rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-900 px-3 py-2 text-xs hover:bg-slate-50 dark:hover:bg-white/5">
                                                + Add
                                            </button>
                                        </div>
                                    ...
                                    <div class="grid grid-cols-2 gap-2">
                                        <div>
                                            <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Date <span class="text-rose-600">*</span></label>
                                            <input type="date" name="transactions_date" value="{{ $oldFor('transactions_date', 'loan_taken') ?: $today }}" class="{{ $inputClass }}">
                                        </div>
                                        <div>
                                            <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Receipt No</label>
                                            <input type="text" name="recipt_no" value="{{ $oldFor('recipt_no', 'loan_taken') }}" class="{{ $inputClass }}">
                                        </div>
                                    </div>

                                    <div>
                                        <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Note</label>
                                        <input type="text" name="note" value="{{ $oldFor('note', 'loan_taken') }}" class="{{ $inputClass }}">
                                    </div>
                            ...
                            <div x-show="tab==='loan_repayment'" x-cloak>
                                <form method="POST" action="{{ url('/transactions/quick-store') }}" class="space-y-3">
                                    ...
                                    <div>
                                        <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Lender</label>
                                        <div class="flex items-center gap-2">
                                            <select id="select_loan_repayment_lender" data-entity-select="lenders" name="lender_id" class="{{ $selectClass }}">
                                                <option value="">Select lender</option>
                                                @foreach ($lenders ?? [] as $l)
                                                    @php $label = $l->name ?? ($l->lender_name ?? 'Lender #' . $l->id); @endphp
                                                    <option value="{{ $l->id }}" @selected($oldSelected('lender_id', $l->id, 'loan_repayment'))>{{ $label }}</option>
                                                @endforeach
                                            </select>
                                            <button type="button" @click="openModal('lenders', 'select_loan_repayment_lender')"
                                                class="shrink-0 rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-900 px-3 py-2 text-xs hover:bg-slate-50 dark:hover:bg-white/5">
                                                + Add
                                            </button>
                                        </div>
                                    ...
                                    <div>
                                        <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Amount</label>
                                        <input type="number" step="0.01" name="amount" value="{{ $oldFor('amount', 'loan_repayment') }}" class="{{ $inputClass }}">
                                        <p class="text-[11px] text-slate-500 mt-1">Ledger: Repayment = <b>OUT (debit)</b>.</p>
                                    </div>

                                    <div>
                                        <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Account <span class="text-rose-600">*</span></label>
                                        <div class="flex items-center gap-2">
                                            <select id="select_loan_repayment_account" data-entity-select="accounts" name="account_id" class="{{ $selectClass }}">
                                                <option value="">Select account</option>
                                                @foreach ($accounts ?? [] as $acc)
                                                    <option value="{{ $acc->id }}" @selected($oldSelected('account_id', $acc->id, 'loan_repayment'))>{{ $acc->name ?? 'Account #' . $acc->id }}</option>
                                                @endforeach
                                            </select>
                                            <button type="button" @click="openModal('accounts', 'select_loan_repayment_account')"
                                                class="shrink-0 rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-900 px-3 py-2 text-xs hover:bg-slate-50 dark:hover:bg-white/5">
                                                + Add
                                            </button>
                                        </div>
                                    ...
                                    <div class="grid grid-cols-2 gap-2">
                                        <div>
                                            <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Date <span class="text-rose-600">*</span></label>
                                            <input type="date" name="transactions_date" value="{{ $oldFor('transactions_date', 'loan_repayment') ?: $today }}" class="{{ $inputClass }}">
                                        </div>
                                        <div>
                                            <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Receipt No</label>
                                            <input type="text" name="recipt_no" value="{{ $oldFor('recipt_no', 'loan_repayment') }}" class="{{ $inputClass }}">
                                        </div>
                                    </div>

                                    <div>
                                        <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Note</label>
                                        <input type="text" name="note" value="{{ $oldFor('note', 'loan_repayment') }}" class="{{ $inputClass }}">
                                    </div>
                            ...
                            <div x-show="tab==='expense'" x-cloak>
                                <form method="POST" action="{{ url('/transactions/quick-store') }}" class="space-y-3">
                                    ...
                                    <div>
                                        <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Expense Title</label>
                                        <input type="text" name="title" value="{{ $oldFor('title', 'expense') }}" placeholder="e.g., Electricity bill" class="{{ $inputClass }}">
                                    </div>

                                    <div class="grid grid-cols-2 gap-2">
                                        <div>
                                            <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Category <span class="text-rose-600">*</span></label>
                                            <div class="flex items-center gap-2">
                                                <select id="select_expense_category" data-entity-select="catagories" name="catagory_id" class="{{ $selectClass }}">
                                                    <option value="">Select</option>
                                                    @foreach ($catagories ?? [] as $c)
                                                        @php $nm = $c->name ?? ($c->title ?? ('Category #'.$c->id)); @endphp
                                                        <option value="{{ $c->id }}" @selected($oldSelected('catagory_id', $c->id, 'expense'))>{{ $nm }}</option>
                                                    @endforeach
                                                </select>
                                                <button type="button" @click="openModal('catagories', 'select_expense_category')"
                                                    class="shrink-0 rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-900 px-3 py-2 text-xs hover:bg-slate-50 dark:hover:bg-white/5">
                                                    + Add
                                                </button>
                                            </div>
                                        ...
                                        <div>
                                            <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Expense Head <span class="text-rose-600">*</span></label>
                                            <div class="flex items-center gap-2">
                                                <select id="select_expense_head" data-entity-select="expens" name="expens_id" class="{{ $selectClass }}">
                                                    <option value="">Select</option>
                                                    @foreach ($expensList ?? [] as $e)
                                                        @php $nm = $e->name ?? ($e->title ?? ('Head #'.$e->id)); @endphp
                                                        <option value="{{ $e->id }}" @selected($oldSelected('expens_id', $e->id, 'expense'))>{{ $nm }}</option>
                                                    @endforeach
                                                </select>
                                                <button type="button" @click="openModal('expens', 'select_expense_head')"
                                                    class="shrink-0 rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-900 px-3 py-2 text-xs hover:bg-slate-50 dark:hover:bg-white/5">
                                                    + Add
                                                </button>
                                            </div>
                                    ...
                                    <div>
                                        <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Amount</label>
                                        <input type="number" step="0.01" name="amount" value="{{ $oldFor('amount', 'expense') }}" class="{{ $inputClass }}">
                                        <p class="text-[11px] text-slate-500 mt-1">Ledger: Expense = <b>OUT (debit)</b>.</p>
                                    </div>

                                    <div>
                                        <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Account <span class="text-rose-600">*</span></label>
                                        <div class="flex items-center gap-2">
                                            <select id="select_expense_account" data-entity-select="accounts" name="account_id" class="{{ $selectClass }}">
                                                <option value="">Select account</option>
                                                @foreach ($accounts ?? [] as $acc)
                                                    <option value="{{ $acc->id }}" @selected($oldSelected('account_id', $acc->id, 'expense'))>{{ $acc->name ?? 'Account #' . $acc->id }}</option>
                                                @endforeach
                                            </select>
                                            <button type="button" @click="openModal('accounts', 'select_expense_account')"
                                                class="shrink-0 rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-900 px-3 py-2 text-xs hover:bg-slate-50 dark:hover:bg-white/5">
                                                + Add
                                            </button>
                                        </div>
                                    ...
                                    <div class="grid grid-cols-2 gap-2">
                                        <div>
                                            <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Date <span class="text-rose-600">*</span></label>
                                            <input type="date" name="transactions_date" value="{{ $oldFor('transactions_date', 'expense') ?: $today }}" class="{{ $inputClass }}">
                                        </div>
                                        <div>
                                            <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Receipt No</label>
                                            <input type="text" name="recipt_no" value="{{ $oldFor('recipt_no', 'expense') }}" class="{{ $inputClass }}">
                                        </div>
                                    </div>

                                    <div>
                                        <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Note</label>
                                        <input type="text" name="note" value="{{ $oldFor('note', 'expense') }}" class="{{ $inputClass }}">
                                    </div>
                            ...
                            <div x-show="tab==='income'" x-cloak>
                                <form method="POST" action="{{ url('/transactions/quick-store') }}" class="space-y-3">
                                    ...
                                    <div>
                                        <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Income Title</label>
                                        <input type="text" name="title" value="{{ $oldFor('title', 'income') }}" placeholder="e.g., Service income" class="{{ $inputClass }}">
                                    </div>

                                    <div class="grid grid-cols-2 gap-2">
                                        <div>
                                            <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Income Head <span class="text-rose-600">*</span></label>
                                            <div class="flex items-center gap-2">
                                                <select id="select_income_head" data-entity-select="incomes" name="income_id" class="{{ $selectClass }}">
                                                    <option value="">Select</option>
                                                    @foreach ($incomesList ?? [] as $i)
                                                        @php $nm = $i->name ?? ($i->title ?? ('Income #'.$i->id)); @endphp
                                                        <option value="{{ $i->id }}" @selected($oldSelected('income_id', $i->id, 'income'))>{{ $nm }}</option>
                                                    @endforeach
                                                </select>
                                                <button type="button" @click="openModal('incomes', 'select_income_head')"
                                                    class="shrink-0 rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-900 px-3 py-2 text-xs hover:bg-slate-50 dark:hover:bg-white/5">
                                                    + Add
                                                </button>
                                            </div>
                                        ...
                                        <div>
                                            <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Category (optional)</label>
                                            <div class="flex items-center gap-2">
                                                <select id="select_income_category" data-entity-select="catagories" name="catagory_id" class="{{ $selectClass }}">
                                                    <option value="">Select</option>
                                                    @foreach ($catagories ?? [] as $c)
                                                        @php $nm = $c->name ?? ($c->title ?? ('Category #'.$c->id)); @endphp
                                                        <option value="{{ $c->id }}" @selected($oldSelected('catagory_id', $c->id, 'income'))>{{ $nm }}</option>
                                                    @endforeach
                                                </select>
                                                <button type="button" @click="openModal('catagories', 'select_income_category')"
                                                    class="shrink-0 rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-900 px-3 py-2 text-xs hover:bg-slate-50 dark:hover:bg-white/5">
                                                    + Add
                                                </button>
                                            </div>
                                    ...
                                    <div>
                                        <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Amount</label>
                                        <input type="number" step="0.01" name="amount" value="{{ $oldFor('amount', 'income') }}" class="{{ $inputClass }}">
                                        <p class="text-[11px] text-slate-500 mt-1">Ledger: Income = <b>IN (credit)</b>.</p>
                                    </div>

                                    <div>
                                        <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Account <span class="text-rose-600">*</span></label>
                                        <div class="flex items-center gap-2">
                                            <select id="select_income_account" data-entity-select="accounts" name="account_id" class="{{ $selectClass }}">
                                                <option value="">Select account</option>
                                                @foreach ($accounts ?? [] as $acc)
                                                    <option value="{{ $acc->id }}" @selected($oldSelected('account_id', $acc->id, 'income'))>{{ $acc->name ?? 'Account #' . $acc->id }}</option>
                                                @endforeach
                                            </select>
                                            <button type="button" @click="openModal('accounts', 'select_income_account')"
                                                class="shrink-0 rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-900 px-3 py-2 text-xs hover:bg-slate-50 dark:hover:bg-white/5">
                                                + Add
                                            </button>
                                        </div>
                                    ...
                                    <div class="grid grid-cols-2 gap-2">
                                        <div>
                                            <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Date <span class="text-rose-600">*</span></label>
                                            <input type="date" name="transactions_date" value="{{ $oldFor('transactions_date', 'income') ?: $today }}" class="{{ $inputClass }}">
                                        </div>
                                        <div>
                                            <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Receipt No</label>
                                            <input type="text" name="recipt_no" value="{{ $oldFor('recipt_no', 'income') }}" class="{{ $inputClass }}">
                                        </div>
                                    </div>

                                    <div>
                                        <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Note</label>
                                        <input type="text" name="note" value="{{ $oldFor('note', 'income') }}" class="{{ $inputClass }}">
                                    </div>
                        ...
                        <div x-show="modalOpen" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
                            ...
                            <div class="relative w-full max-w-md rounded-2xl bg-white dark:bg-slate-950 border border-slate-200 dark:border-white/10 shadow-xl overflow-hidden">
                                <div class="px-4 py-3 border-b border-slate-200 dark:border-white/10 flex items-center justify-between">
                                    <div>
                                        <div class="text-sm font-semibold text-slate-800 dark:text-slate-100" x-text="modalTitle"></div>
                                ...
                                <div class="p-4 space-y-3">
                                    <template x-if="modalErrors.length">
                                        <div class="rounded-xl border border-rose-200 bg-rose-50 px-3 py-2 text-rose-800 text-xs">
                                            <div class="font-semibold mb-1">Fix these:</div>
                                            <ul class="list-disc list-inside space-y-1">
                                    ...
                                    <div x-show="modalEntity!=='students'">
                                        ...
                                        <input type="text" x-model="modalForm.name" class="{{ $inputClass }}" placeholder="Enter name">
                                    </div>
                                    ...
                                    <div x-show="modalEntity==='students'" x-cloak class="space-y-2">
                                        <div>
                                            ...
                                            <input type="text" x-model="modalForm.full_name" class="{{ $inputClass }}" placeholder="Student full name">
                                        </div>

                                        <div>
                                            <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Father Name</label>
                                            <input type="text" x-model="modalForm.father_name" class="{{ $inputClass }}" placeholder="Father name (optional)">
                                        </div>

                                        <div class="grid grid-cols-2 gap-2">
                                            <div>
                                                <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">DOB</label>
                                                <input type="date" x-model="modalForm.dob" class="{{ $inputClass }}">
                                            </div>
                                            <div>
                                                <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Age</label>
                                                <input type="number" x-model="modalForm.age" class="{{ $inputClass }}">
                                            </div>
                                        </div>

                                        <div class="grid grid-cols-2 gap-2">
                                            <div>
                                                <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Roll</label>
                                                <input type="number" x-model="modalForm.roll" class="{{ $inputClass }}">
                                            </div>
                                            <div>
                                                <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Mobile</label>
                                                <input type="text" x-model="modalForm.mobile" class="{{ $inputClass }}">
                                            </div>
                                        </div>

                                        <div class="grid grid-cols-2 gap-2">
                                            <div>
                                                <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Email (optional)</label>
                                                <input type="email" x-model="modalForm.email" class="{{ $inputClass }}">
                                            </div>
                                            <div>
                                                <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Fees Type</label>
                                                <select x-model="modalForm.fees_type_id" class="{{ $selectClass }}">
                                                    <option value="">Select</option>
                                                    @foreach ($feesTypes ?? [] as $ft)
                                                        <option value="{{ $ft->id }}">{{ $ft->name ?? 'Type #' . $ft->id }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                        ...
                                        <div class="grid grid-cols-2 gap-2">
                                            <div>
                                                <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Academic Year</label>
                                                <select x-model="modalForm.academic_year_id" class="{{ $selectClass }}">
                                                    <option value="">Select</option>
                                                    @foreach ($years ?? [] as $y)
                                                        <option value="{{ $y->id }}">{{ $y->name ?? 'Year #' . $y->id }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            <div>
                                        ...
                                        <div>
                                            ...
                                            <select x-model="modalForm.section_id" class="{{ $selectClass }}">
                                                <option value="">Select</option>
                                                @foreach ($sections ?? [] as $sec)
                                                    <option value="{{ $sec->id }}">{{ $sec->name ?? 'Section #' . $sec->id }}</option>
                                                @endforeach
                                            </select>
                                        </div>

                                        <label class="inline-flex items-center gap-2 text-xs text-slate-700 dark:text-slate-200 mt-1">
                                            <input type="checkbox" x-model="modalForm.isActived" class="rounded border-slate-300 dark:border-white/20">
                                            Active
                                        </label>
                                    </div>
                                    ...
                                    <div x-show="modalEntity==='donors'" x-cloak class="grid grid-cols-2 gap-2">
                                        <div>
                                            <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Mobile</label>
                                            <input type="text" x-model="modalForm.mobile" class="{{ $inputClass }}" placeholder="01xxxxxxxxx">
                                        </div>
                                        <div>
                                            <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Email (optional)</label>
                                            <input type="email" x-model="modalForm.email" class="{{ $inputClass }}" placeholder="email@example.com">
                                        </div>
                                    </div>

                                    {{-- Accounts --}}
                                    <div x-show="modalEntity==='accounts'" x-cloak>
                                        <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Account Details</label>
                                        <input type="text" x-model="modalForm.account_details" class="{{ $inputClass }}" placeholder="e.g., Cash in hand">
                                        <div class="mt-2">
                                            <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Account Number (optional)</label>
                                            <input type="text" x-model="modalForm.account_number" class="{{ $inputClass }}" placeholder="Leave blank for auto">
                                        </div>
                                    </div>
                                    ...
                                    <div x-show="modalEntity==='lenders'" x-cloak class="space-y-2">
                                        <div class="grid grid-cols-2 gap-2">
                                            <div>
                                                <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Phone</label>
                                                <input type="text" x-model="modalForm.phone" class="{{ $inputClass }}" placeholder="01xxxxxxxxx">
                                            </div>
                                            <div>
                                                <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Email (optional)</label>
                                                <input type="email" x-model="modalForm.email" class="{{ $inputClass }}" placeholder="email@example.com">
                                        ...
                                        <div>
                                            ...
                                            <input type="text" x-model="modalForm.address" class="{{ $inputClass }}" placeholder="Address">
                                        </div>
                                        <div>
                                            <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Bank Details</label>
                                            <input type="text" x-model="modalForm.bank_detils" class="{{ $inputClass }}" placeholder="Bank details">
                                        </div>
                                    </div>
                                    ...
                                    <p class="text-[11px] text-slate-500">
                                        AJAX: <code class="px-1 py-0.5 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded">POST</code>
                ...
                async submitModal() {
                    ...
                    try {
                        ...
                        if (this.modalEntity === 'students') {
                            ...
                            let payload = { name: this.modalForm.name };
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const feeNames = ['admission_fee','monthly_fees','boarding_fees','management_fees','exam_fees','others_fees'];
