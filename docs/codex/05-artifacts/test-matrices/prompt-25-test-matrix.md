...
This artifact is the durable version of the prompt-25 test-matrix analysis. It carries forward the approved prompt-23 schema plan and the prompt-24 migration posture: additive schema first, classification before mutation, and no guessing on ambiguous identity or linkage rows.
