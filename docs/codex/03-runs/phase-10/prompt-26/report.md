...
This prompt turns the approved prompt-12 through prompt-25 planning set into an additive-first rollout and risk plan. No contradiction was found with prompt-23 schema constraints, prompt-24 migration posture, or prompt-25 rollout blockers. The main rollout rule is staged cutover: land shared foundations dark, prove classification-first backfill and eligibility boundaries, then turn on business-facing surfaces in the order that creates value without reopening the highest-risk data or auth problems too early.

The prompt-25 durable test matrix is the gating framework for this rollout plan. Every rollout wave below assumes cumulative regression coverage, not a fresh risk reset.
...
- Business intent: enable the rest of the program safely, reduce future rework, and create the auth and state platform needed for every later slice.
...
- Business intent: unlock new donation intake and narrow receipt/status value with less scope than a full donor portal rollout.
...
- Business intent: give confirmed eligible donors self-service history after payment-domain correctness is already proven.
...
- Business intent: deliver visible guardian value early with much less sensitive-data risk than student or invoice access.
...
- Business intent: add the highest-value guardian self-service capability only after the safer guardian layer is proven.
...
- Business intent: improve convenience and unified-account usability without making identity or routing the first moving piece.
...
- Business intent: finish polish and release confidence only after the product model is already stable.
